...
export function EntryModal() {
  ...
  useEffect(() => {
    ...
    setDetailError("");
    void fetchMediaDetailById(entry.anilistId, controller.signal)
      .then((media) => {
        if (!cancelled) {
          setDetail(media);
  ...
  useEffect(() => {
    ...
    function onKey(ev: KeyboardEvent) {
      if (ev.key === "Escape") setActiveEntryId(null);
    }
  ...
  const synopsis = plainSynopsis(detail?.description);
  const shortSynopsis = synopsis.length > 420 ? synopsis.slice(0, 420).trimEnd() + "…" : synopsis;
  const showToggle = synopsis.length > 420;
  const embedUrl = trailerEmbedUrl(detail?.trailer ?? null);
  const watchUrl = trailerWatchUrl(detail?.trailer ?? null);
  const anilistUrl = detail?.siteUrl || `https://anilist.co/anime/${entry.anilistId}`;
  const airing = nextAiringText(entry);
  const seasonLabel = detail?.season ? SEASON_LABELS[detail.season] || detail.season : null;
  const score = detail?.averageScore ?? null;
  const altTitles = detail
    ? [detail.title.english, detail.title.native].filter(
        (t): t is string => Boolean(t) && t !== entry.title,
      )
    : [];
    ...
    >
      ...
      >
        ...
        <button
          type="button"
          aria-label="Fermer"
          className="absolute top-3 right-3 z-10 rounded-full border border-line/80 bg-raised/90 p-1.5 text-dim backdrop-blur hover:text-ink"
        ...
        <div className="p-5 sm:p-6">
          <div className={cn("mb-5 flex gap-3.5", detail?.bannerImage ? "-mt-12 relative" : "")}>
            <Cover
              src={entry.image}
              title={entry.title}
            ...
            <div className="min-w-0 flex-1 pr-8 pt-1">
              ...
              {altTitles.length > 0 ? (
                <p className="mt-0.5 truncate text-[11.5px] text-dim">{altTitles.join(" · ")}</p>
              ) : null}
              ...
              <div className="mt-2 flex flex-wrap items-center gap-2">
                <span
                  className="rounded-full px-2 py-0.5 text-[10.5px] font-bold text-bg"
                  style={{ background: meta.color }}
                >
                  {meta.label}
                </span>
                {score != null ? (
                  <span className="inline-flex items-center gap-1 rounded-full border border-line bg-bg px-2 py-0.5 text-[10.5px] font-semibold text-lime">
                    <Star className="size-3 fill-lime" />
                    {(score / 10).toFixed(1)} AniList
                  </span>
                ) : null}
                {airing ? (
                  <span className="rounded-full border border-lime/30 bg-lime/10 px-2 py-0.5 text-[10.5px] font-semibold text-lime">
                    {airing}
                  </span>
                ) : null}
              </div>
          ...
          <div className="mb-5 space-y-3">
            {detailLoading ? (
              <p className="text-xs text-dim">Chargement du synopsis…</p>
            ) : detailError ? (
              <p className="text-xs text-dim">{detailError}</p>
            ) : synopsis ? (
              <div>
                <div className="mb-1.5 text-[11.5px] font-semibold text-dim">Synopsis</div>
                <p className="whitespace-pre-wrap text-[13px] leading-relaxed text-ink/90">
                  {synopsisExpanded || !showToggle ? synopsis : shortSynopsis}
                </p>
                {showToggle ? (
                  <button
                    type="button"
                    className="mt-1 text-[12px] font-semibold text-lime hover:underline"
            ...
            {embedUrl ? (
              <div>
                <div className="mb-1.5 flex items-center gap-1.5 text-[11.5px] font-semibold text-dim">
                  <Film className="size-3.5" /> Bande-annonce
                </div>
                <div className="aspect-video overflow-hidden rounded-[10px] border border-line bg-bg">
                  <iframe
                    src={embedUrl}
                    title={`Bande-annonce — ${entry.title}`}
              ...
              <a
                ...
                className="inline-flex items-center gap-1.5 text-[12.5px] font-semibold text-lime hover:underline"
              >
                <Film className="size-3.5" /> Voir la bande-annonce
          ...
          {confirming ? (
            ...
            <button
              type="button"
              className="mt-2 flex w-full items-center justify-center gap-2 rounded-[9px] border border-crimson py-2.5 text-[13px] font-semibold text-crimson hover:bg-crimson/10"
